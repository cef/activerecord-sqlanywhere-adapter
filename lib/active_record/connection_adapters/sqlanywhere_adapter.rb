    ...
    class SQLAnywhereAdapter < AbstractAdapter
      ...
      def columns(table_name, name = nil) #:nodoc:
        table_structure(table_name).map do |field|
          ...
          SQLAnywhereColumn.new(field['name'], field['default'], SQLAnywhereColumnType.new(field['domain']), (field['nulls'] == 1))
        end
      end
      ...
      def exec_query(sql, name = 'SQL', binds = [])
        ...
        ActiveRecord::Result.new(keys, result.collect { |r| r.values })
      end
      ...
      def query(sql)
        ...
        if record.blank?
          ...
          keys = record.first.keys
        end
        result = ActiveRecord::Result.new(keys, record.collect { |r| r.values })
        return result
      end
    end
  end
...
class SQLAnywhereColumnType < ActiveRecord::Type::Value
  
  attr_accessor :type, :limit, :precision, :scale
  
  def initialize(type, precision=nil, scale=nil)
    parse_type(type)
  end
